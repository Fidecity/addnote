...
func (bs *XWCBlockScanner) forkBlockNotify(block *Block) {
...
//newBlockNotify 获得新区块后，通知给观测者
func (bs *XWCBlockScanner) newBlockNotify(block *Block) {
	header := ParseHeader(block)
	bs.NewBlockNotify(header)
}
...
func (bs *XWCBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *XWCBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, blockTime int64, transaction *types.Transaction, scanTargetFunc openwallet.BlockScanTargetFunc) ExtractResult {
	...
	for _, operation := range transaction.Operations {
		...
		if transferOperation, ok := operation.(*types.TransferOperation); ok {
			...
			from := transferOperation.FromAddr
			to := transferOperation.ToAddr
...
func (bs *XWCBlockScanner) extractTxInput(operation *types.TransferOperation, txExtractData *openwallet.TxExtractData) {
...
func (bs *XWCBlockScanner) extractTxOutput(operation *types.TransferOperation, txExtractData *openwallet.TxExtractData) {
...
func (bs *XWCBlockScanner) newExtractDataNotify(height uint64, extractData map[string][]*openwallet.TxExtractData) error {
...
func (bs *XWCBlockScanner) scanBlock(height uint64) (*Block, error) {
...
func (bs *XWCBlockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
	...
	tx, err := bs.wm.Api.GetTransaction(txid)
	if err != nil {
		return nil, err
	}

	block, err := bs.wm.Api.GetBlockByHeight(tx.BlockNum)

	result := bs.ExtractTransaction(block.Height, block.BlockID, block.Timestamp.Unix(), tx, scanTargetFunc)
